...
describe('GET /transactions', () => {
  ...
  const endpoint = `${env.protocol}://${env.host}:${env.port}/transactions/xyz`;

  it(
    `
      *** Integration Testing ***

      This endpoint must returns an error object  
      since the resource does not exists on the server.
    ...
    async () => {
      ...
      expect(response.data['status']).toBe(499);
      expect(response.data['message']).toBe('Resource does not exist.');
    }
  );
